...
class ezFBmarquee():
    ...
    def __init__(self, display, font,
        ...
        self._font = font
        self._x = x
        self._y = max(0, (self._device.height - self._font.height()) // 2)
    ...
    def stop(self):
        ...
        self._device.rect(self._x, self._y, self._width, self._height, self._bg)
        if self._verbose:
    ...
    def step(self, steps=1):
        ...
        if self.string is None:
            return False
        # Clear the outframe unconditionally before each step
        self._outframe.fill(0)
        # Do animation step
        res = False
        if (self._pause == 0) and (steps > 0):
            ...
            # Blit the scrollbuffer over outframe at the new offset
            if self._stepping:
